class Solution {
    ...
    */
    public static void placeQueens(int[][]board, int prev_i, int i_ind, int j_ind, int numQs) {
       //check if board state is legal
       if (!legalCheck(board, prev_i, j_ind)) {
           return;
       }
       //if we've placed all the queens, add solution and return
       else if (numQs==board.length){
           solutions.add(convert(board));
           return;
       }
       else {
           int i = i_ind;
           //iterate through columns and recurse when queen is placed
           for(int j =0; j<board[i].length; j++){
               board[i][j]=1;
               placeQueens(board, i,i+1, j, numQs+1);
               board[i][j]=0;
           }
       }
    }
